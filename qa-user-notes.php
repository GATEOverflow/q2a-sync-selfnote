...
class user_notes_page
{
	...
	public function process_request($request)
	{
		...
		$selectspec2 = $this->qa_db_category_nav_with_usernotes_selectspec($categoryslugs, $userid);
	...
	private function qa_db_category_nav_with_usernotes_selectspec($categoryslugs, $userid, $full = true)
	{
		...
		$selectspec['columns']['questions_count'] = 'COUNT(DISTINCT usernotes.postid)';
		...
		$selectspec['source'] = str_replace(
			...
			AND ^posts.type = \'Q\'
			 LEFT JOIN ^usernote AS usernotes 
				ON usernotes.postid = ^posts.postid 
			   AND usernotes.userid = ' . (int)$userid . '
			 GROUP BY ^categories.categoryid',
			$selectspec['source']
		);
